"""
Smart Grid Demand Response — Data Models
==========================================
Action, Observation, and StepResult types for a city power grid
demand response environment designed for LLM agents.
"""
...
class Observation(BaseModel):
    ...
    grid_frequency_hz: float = Field(
        default=50.0,
        description="Grid frequency in Hz. Normal=50.0, Warning<49.5, Blackout<49.0"
    )
    alert_level: str = Field(
        default="normal",
        description="Grid alert: 'normal', 'warning', 'critical', 'blackout'"
    )
    ...
    # ── Weather ──
    weather: str = Field(default="clear", description="Weather condition: clear, cloudy, heatwave, storm, monsoon")
...
class Action(BaseModel):
    """
    Agent's demand response decision. Specifies which loads to curtail
    and by how many MW. LLM-friendly JSON dict format.

    Example:
        {"curtailments": {"steel_plant": 20.0, "shopping_mall_1": 5.0}}
    """
